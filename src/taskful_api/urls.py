...
def dummy_authorize(request):
    return HttpResponse("Dummy authorize view")

def dummy_token_view(request):
    return HttpResponse("Dummy token endpoint")
auth_api_urls=[
...
urlpatterns = [
    ...
    path('authorize/',dummy_authorize,name='authorize'),
    path('token/',dummy_token_view,name='token'),
]
